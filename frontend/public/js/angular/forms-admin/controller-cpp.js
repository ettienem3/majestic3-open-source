...
formsAdminControllers.controller('CPPFormCtrl', ['$scope', '$rootScope', '$route', '$routeParams', '$window', 'FormAdminPageService', 'promiseTracker', function CPPFormCtrl($scope, $rootScope, $route, $routeParams, $window, FormAdminPageService, promiseTracker, formlyVersion) {
	...
	$scope.vm.submitForm = function () {
		...
		//submit the data
		if (typeof $rootScope.edit_cpp_id != 'undefined' && $rootScope.edit_cpp_id > 0)
		{
			//update a form
			objData.acrq = 'update-form-config-data';
			objData.id = $rootScope.edit_cpp_id;
		} else {
			//create a new form
			objData.acrq = 'create-cpp-form';
		}//end if
		...
		var $promise = FormAdminPageService.post(objData,
			function success(response) {
				...
				if (typeof $rootScope.edit_cpp_id != 'undefined' && response.error == 0)
				{
					$scope.vm.model = {};
					delete $rootScope.edit_cpp_id;
					...
					doMessageAlert('Contact Profile form has been created', '<p>The form has been created, you can start allocating fields to the form now.</p>');
				}//end if
			},
	...
	$rootScope.$on('loadCPPFormAdminFields', function () {
		//load form data
		if (typeof $rootScope.edit_cpp_id != 'undefined' && $rootScope.edit_cpp_id > 0)
		{
			var form_id = $rootScope.edit_cpp_id;
			var objRequest = {
					'fid': form_id,
		...
		$scope.vm.fields = formAdminFormSimple('__cpp');
		if ($scope.vm.fields.length > 0)
		{
			return;
		}//end if
		
		var $promise = FormAdminPageService.get({acrq: 'load-admin-form-cpp'}, 
			function success(response) {
				logToConsole(response);
