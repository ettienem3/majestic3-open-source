...
class PanelContactsWidgetStatusesVsContactsProcessorModel extends AbstractPanelProcessorModel implements InterfacePanelsProcessor
{
	/**
	 * (non-PHPdoc)
	...
	public function processPanel(FrontPanelsPanelEntity $objPanel)
	{
		...
		$html .= 	"jQuery(function () {
					    jQuery('#$html_id').highcharts({
					        data: {
					            table: 'status-vs-contacts-count'
					        },
					        chart: {
					            type: 'pie'
					        },
					        title: {
					            text: 'Contacts per Status'
					        },
					        yAxis: {
					            allowDecimals: false,
					        },
		...
		$html .= "</script>";
		$html .= "<table id=\"status-vs-contacts-count\" style=\"display: none;\">";
// 		$html .= 	"<thead>";
// 		$html .= 	"</thead>";
		$html .= 	"<tbody>";
		...
		foreach ($objPanel->get("panel_read_data") as $k => $objData)
		{
			if ($objData->status == "")
			{
				$objData->status = "Not Specified";
			}//end if

			$html .= "<tr>";
			$html .=	"<th>" . $objData->status . "</th>";
